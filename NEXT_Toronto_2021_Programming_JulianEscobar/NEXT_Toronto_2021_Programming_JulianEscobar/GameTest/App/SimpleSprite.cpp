...
{            
    ...
    if (DebugManager::s_debugMode)
    {
        DebugManager::DrawRect(getRect(), { 1.0f,1.0f,1.0f });
    }
}
...
const CRect CSimpleSprite::getRect()
{
    CRect rect = CRect(m_xpos, m_ypos, m_width * 0.5f, m_height * 0.5f);
    return rect;
}
